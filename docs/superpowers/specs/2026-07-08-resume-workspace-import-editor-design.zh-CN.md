...
> 2026-07-09 状态更新：本文件中把首次创建、导入、编辑和预览集中在单页 Resume Workspace 的设计已被分步简历创建向导取代。向导的确认规范和预览图位于 [全站统一布局设计](2026-07-09-unified-site-layout-design.zh-CN.md#简历创建向导)。本文件关于每用户每语言一份正式简历、Markdown 编辑器、树状大纲、AI 同语言合并、section 拆分和向量生成的设计继续适用于保存后的高级 Resume Workspace。

...
public sealed class LocalBgeRerankService : IRerankService, IDisposable
{
    ...
    {
        var modelDirectory = ResolveModelDirectory(environment, configuration);
        var modelPath = ResolvePath(modelDirectory, configuration["Rerank:ModelPath"] ?? DefaultModelFileName);
        var tokenizerPath = ResolvePath(modelDirectory, configuration["Rerank:TokenizerPath"] ?? DefaultTokenizerFileName);
